...
function updateStatistics() {
  ...
  document.getElementById('avgChange').textContent =
    `₹${(totalExpenses / totalTransactions || 0).toFixed(2)} per transaction`;
  document.getElementById('groupChange').textContent =
    `${activeGroups} group${activeGroups !== 1 ? 's' : ''}`;
  document.getElementById('transactionChange').textContent =
    `${totalTransactions} expense${totalTransactions !== 1 ? 's' : ''}`;
}
...
function updateCategoryBreakdown() {
  ...
  if (total === 0) {
    container.innerHTML =
      '<p style="text-align: center; color: rgba(255,255,255,0.6);">No expenses in this period</p>';
    return;
  }
