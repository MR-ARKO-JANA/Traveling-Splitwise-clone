...
async function loadGroupExpenses(groupId, groupName) {
    try {
        ...
        document.getElementById("expenseCardTitle").innerHTML = `<i class="fas fa-list"></i> ${groupName} Expenses`;
...
async function handleAddExpense() {
    ...
    try {
        await dashboardService.addExpense({ description: desc, amount, groupId: selectedGroupId, category: cat });
...
function updateNoExpensesHint(data) {
    const isZero = parseFloat(data.get) === 0 && parseFloat(data.pay) === 0 && parseFloat(data.total) === 0;
...
async function loadRecentActivity() {
    try {
        ...
        msg.style.display = "none";
        list.innerHTML = data.recentExpenses.map(exp => `
...
function getCategoryIcon(category) {
    const icons = { food: "fas fa-utensils", transport: "fas fa-car", accommodation: "fas fa-bed", entertainment: "fas fa-film", utilities: "fas fa-bolt", other: "fas fa-receipt" };
...
async function handleCreateGroup() {
    ...
    try {
        await dashboardService.createGroup({ name, description: document.getElementById("groupDesc").value.trim(), emails: document.getElementById("groupMembers").value.split(",").map(m => m.trim()).filter(m => m) });
...
async function handleDeleteGroup(e, id, name) {
    ...
    try {
        ...
        if (selectedGroupId === id) { selectedGroupId = null; updateExpenseFormState(); }
        await refreshDashboard();
    } catch (err) {
        ui.showNotification(err.message, "error");
    }
...
function updateInviteGroupOptions(groups) {
    ...
    if (select) select.innerHTML = '<option value="">Select a group</option>' + groups.map(g => `<option value="${g._id}">${ui.escapeHtml(g.name)}</option>`).join('');
}